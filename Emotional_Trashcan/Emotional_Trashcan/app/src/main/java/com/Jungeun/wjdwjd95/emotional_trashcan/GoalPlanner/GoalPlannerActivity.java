...
public class GoalPlannerActivity extends AppCompatActivity implements GoalView{
    ...
    public void BtnHistory_Click(){
        {
            ...
            if(list != null && list.size() > 0)
            {
                if(list.size() == 1)
                {
                    if(list.get(0).getStatus().equals(Constants.GOAL_STATUS_ING)) {
                ...
                for(int i =0; i <list.size(); i++)
                {
                    ...
                    TextView Comfirmdate = new TextView(GoalPlannerActivity.this);
                    lp = new LinearLayout.LayoutParams(0,ViewGroup.LayoutParams.WRAP_CONTENT);
                    lp.setMargins(5,5,5,5);
                    lp.weight = 2;
                    Comfirmdate.setText(list.get(i).getComfirmdate());
                    Comfirmdate.setLayoutParams(lp);
                    lp = new LinearLayout.LayoutParams(0,ViewGroup.LayoutParams.WRAP_CONTENT);
                    ...
                    linearLayout.addView(Comfirmdate);
    ...
    public void getGoal()
    {
        ...
        {
            now = new Date();
            GoalEntity goaldateEntity = mPresenter.getGoal_ifexistatdate(Constants.dateFormat.format(now));
