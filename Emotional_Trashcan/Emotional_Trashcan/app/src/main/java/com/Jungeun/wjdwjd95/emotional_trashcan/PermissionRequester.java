        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            if (permissionCheck == PackageManager.PERMISSION_DENIED) {
                ...
                if (context.shouldShowRequestPermissionRationale(permission)) {
                    ...
                    /* * 권한의 취득 요청을 처음 할 때 * 권한의 취득을 요청한다. * 취득하고자 하는 권한을 배열에 넣고 요청한다. * 뒤에 들어가는 파라미터(1000)는 onRequestPermissionsResult() 에서 권한 취득 결과에서 사용된다. * startActiviryForResult의 Request Code와 유사함. */
                    context.requestPermissions(new String[]{permission}, requestCode);
                    return REQUEST_PERMISSION;
                }
...
//출처: http://developtip.tistory.com/78 [Developer's Tip]